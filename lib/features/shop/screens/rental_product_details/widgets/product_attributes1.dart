...
import 'bottom_add_to_cart_widget1.dart';

class EProductAttributes1 extends StatefulWidget {
  const EProductAttributes1({
    Key? key,
  ...
  final String colour, price, fabric, details, status, id;

  @override
  State<EProductAttributes1> createState() => _EProductAttributes1State();
}

class _EProductAttributes1State extends State<EProductAttributes1> {
  ...
  Future<void> _fetchSubCategories() async {
    final ref = FirebaseDatabase.instance.ref().child("Project/RentProduct/${widget.id}");
    ...
    if (recdata != null) {
      ...
      final colorData = recdata['RentProduct_colour'] as String?;

      if (colorData != null) {
        colors = recdata.entries
            .where((entry) => entry.key == 'RentProduct_colour')
                ...
                children: [
                  ...
                  Column(
                    ...
                    children: [
                      Row(
                        children: [
                          const EProductTitleText(title: 'Deposit : ', smallSize: true),
                          const SizedBox(width: ESizes.spaceBtwItems),
                          EProductPriceText(price: "${double.parse(widget.price) * 3  }"),
                        ],
                      ),
                      Row(
                        children: [
                      ...
                      Row(
                        children: [
                          ...
                          const SizedBox(width: ESizes.spaceBtwItems),
                          Text(widget.status, style: Theme.of(context).textTheme.titleMedium),
                        ],
                      ),
                    ],
                  ),
